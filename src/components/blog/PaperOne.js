...
class PaperOne extends Component {
	constructor(props) {
		super(props);
	}

	handleChange(text) {
		this.props.dispatch(setPostText("paper1", text));
	}

	toggleEditing() {
		this.props.dispatch(togglePostEditing("paper1"));
	}

	render() {
		const post = this.props.blog.posts.paper1;
